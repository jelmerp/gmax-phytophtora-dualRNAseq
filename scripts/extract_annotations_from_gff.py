    ...
    for line in open(gff): #looping through GFF file
        ...
        if ((d[0] != '#') and (d != '')): # skip both commented and empty lines
            ...
            if d[2] in gene_feature: # check if gene feature (3rd column of gff file) is present in list of features used in quantification
                      ...
                      #ID_pos_host = [pos.split('=') for pos in d1 if pos.startswith(gene_attribute)][0][1] #! JP commented out
                      # extract gene type 
                      ...
                      # extract gene_id
                      ID_gene_pos_host = [pos.split(' ') for pos in d1 if pos.startswith('gene_id')][0][0].split("=")[1]  #! JP - NOTE: gene_id is hardcoded here!
                      # extract gene_name
...
if args.organism == 'pathogen': 
      ...
      # remove duplicates that occur in case of exons. Exons of the same isoforms have the same 'transcript_id'. #! JP added (copied from 'host' section below)
      gene_types_unique = pd.DataFrame(gene_types).drop_duplicates().to_dict('records')  #! JP added (copied from 'host' section below)
      # create data frame of quantified genes/transcripts with annotations (e.g. gene type and gene name)
      gene_annotations_pathogen_df = pd.DataFrame.from_dict(gene_types_unique) #! JP changed 'gene_types' to 'gene_types_unique'
